...
class RedisCache:
    ...
    def __init__(
        ...
        self.__async_redis: AsyncRedis = config.container.async_redis
        self.__sync_redis: SyncRedis = config.container.sync_redis
            ...
            if inspect.ismethod(func):
                if inspect.iscoroutinefunction(func):
                    async def wrapper(sself, *args, **kwargs):
                        ...
                        _key = key(self.__namespace, *args, **kwargs)
                        _ttl = ttl or self.__ttl
                        client = self.__async_redis.client()
                        async with client:
                            try:
                                async with lock:
                    ...
                    def wrapper(sself, *args, **kwargs):
                        ...
                        _key = key(self.__namespace, *args, **kwargs)
                        _ttl = ttl or self.__ttl
                        client = self.__sync_redis.client()
                        with client:
                            try:
                                with lock:
                                    val = client.get(_key)
