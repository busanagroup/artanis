...
__all__ = ('SortableUID', 'MSSortableUID')
...
SelfT = t.TypeVar("SelfT", bound="SortableUID")
...
class SortableUID:
    ...
    def __init__(self, timestamp: t.Optional[datetime] = None, payload: t.Optional[bytes] = None):
        ...
        timestamp = timestamp.astimezone(timezone.utc) if timestamp is not None else datetime.now(tz=timezone.utc)
        self._uid = self._inner_init(timestamp, _payload)
