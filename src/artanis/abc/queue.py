...
class SyncQueue(AbstractQueue[T], Protocol[T]):
...
class AsyncQueue(AbstractQueue[T], Protocol[T]):
...
class _SyncQueueProxy(SyncQueue[T]):
    ...
    @property
    def maxsize(self) -> int:
        return self._parent._maxsize  # noqa

    @property
    def closed(self) -> bool:
        return self._parent.closed  # noqa
    ...
    def task_done(self) -> None:
        ...
        """
        parent = self._parent
        with parent._sync_tasks_done:  # noqa
            unfinished = parent._unfinished_tasks - 1
            if unfinished <= 0:
                if unfinished < 0:
                    raise ValueError("task_done() called too many times")
                if parent._sync_tasks_done_waiting:  # noqa
                    parent._sync_tasks_done.notify_all()  # noqa
                if parent._async_tasks_done_waiting:  # noqa
                    parent._notify_async(parent._async_tasks_done.notify_all)  # noqa
    ...
    def join(self) -> None:
        ...
        parent = self._parent
        with parent._sync_tasks_done:  # noqa
            while parent._unfinished_tasks:  # noqa
                parent._sync_tasks_done_waiting += 1  # noqa
                try:
                    parent._sync_tasks_done.wait()  # noqa
                finally:
                    parent._sync_tasks_done_waiting -= 1  # noqa

    def qsize(self) -> int:
        """Return the approximate size of the queue (not reliable!)."""
        return self._parent._qsize()  # noqa

    @property
    def unfinished_tasks(self) -> int:
        """Return the number of unfinished tasks."""
        return self._parent._unfinished_tasks  # noqa
    ...
    def empty(self) -> bool:
        ...
        """
        return not self._parent._qsize()  # noqa
    ...
    def put(self, item: T, block: bool = True, timeout: OptFloat = None) -> None:
        ...
        """
        parent = self._parent
        with parent._sync_not_full:  # noqa
            if parent._is_shutdown:  # noqa
            ...
            if parent._maxsize > 0:
                if not block:
                    ...
                    while parent._qsize() >= parent._maxsize:
                        parent._sync_not_full_waiting += 1  # noqa
                        try:
                            parent._sync_not_full.wait()  # noqa
                        finally:
                            parent._sync_not_full_waiting -= 1  # noqa
                        if parent._is_shutdown:  # noqa
                            raise SyncQueueShutDown
                    ...
                    while parent._qsize() >= parent._maxsize:  # noqa
