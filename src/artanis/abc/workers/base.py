...
class AbstractWorker(abc.ABC):
    ...
    @property
    def app(self) -> "ArtanisService":
        """Application instance.

        :return: Application instance.
        """
        if not self._app:
            raise ArtanisError("Worker not initialized")

        return self._app

    @app.setter
    def app(self, app: "ArtanisService") -> None:
...
class WorkerType(abc.ABCMeta):
    ...
    def _is_base(namespace: dict[str, t.Any]) -> bool:
        return namespace.get("__module__") == "artanis.abc.workers" and namespace.get("__qualname__") == "BaseWorker"
