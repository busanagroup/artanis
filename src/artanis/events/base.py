...
from typing_extensions import TypeVar, TypedDict  # needed to get TypeVar(default=...) above python 3.11
...
from .helpers import (  # pyright: ignore[reportPrivateUsage]
...
if TYPE_CHECKING:
    ...
    from .locking import ReentrantLock

logger = logging.getLogger('artbus')

BUS_LOGGING_LEVEL = os.getenv('BUS_LOGGING_LEVEL', 'WARNING').upper()  # WARNING normally, otherwise DEBUG when testing
...
def validate_event_name(s: str) -> str:
    assert not str(s).startswith('_'), f'Invalid event name: {s}'
    return str(s)
...
class EventResult(BaseModel, Generic[T_EventResultType]):
    ...
    def update(self, **kwargs: Any) -> Self:
        ...
        if 'status' in kwargs:
            assert kwargs['status'] in ('pending', 'started', 'completed',
    ...
    def _create_slow_handler_warning_timer(
        ...
        if not should_warn_for_slow_handler:
            return None
        return partial(self._slow_handler_monitor, event=event, eventbus=eventbus,
    ...
    def _on_handler_timeout(self, event: 'BaseEvent[T_EventResultType]') -> EventHandlerTimeoutError:
        ...
        from .logging import log_timeout_tree
    ...
    async def run_handler(
        ...
        handler: NormalizedEventHandlerCallable[
                     T_EventResultType] | None = self.handler._handler_async  # pyright: ignore[reportPrivateUsage]
        if handler is None:
        ...
        try:
            async with eventbus.locks._run_with_handler_lock(eventbus, event,
                                                             self):  # pyright: ignore[reportPrivateUsage]
                ...
                with eventbus._run_with_handler_dispatch_context(event,
                                                                 self.handler_id):  # pyright: ignore[reportPrivateUsage]
                    slow_handler_monitor = self._create_slow_handler_warning_timer(event, eventbus,
    ...
    def log_tree(
        ...
        from .logging import log_event_result_tree
...
class EventAttrDict(TypedDict, total=False):
    source: str
    specversion: str
    datacontenttype: str
    dataschema: str
...
class BaseEvent(BaseModel, Generic[T_EventResultType]):
    ...
    event_handler_timeout: float | None = Field(default=None,
                                                description='Optional per-event handler timeout cap in seconds')
    ...
    def _iter_eventbuses_for_registry(registry_owner: 'EventBus | None' = None) -> list['EventBus']:
        from .eventbus import EventBus, get_current_eventbus
    ...
    def __await__(self) -> Generator[Self, Any, Any]:
        ...
        async def wait_for_handlers_to_complete_then_return_event():
            ...
            from .eventbus import in_handler_context
    ...
    def _normalize_and_validate_event_input(cls, data: Any) -> Any:
        ...
        for key in RESERVED_USER_EVENT_FIELDS:
            if key in params:
                raise ValueError(
                    f'Field "{key}" is reserved for BaseEvent runtime APIs and cannot be set in event payload')
        ...
        for at_key in ('event_created_at', 'event_started_at', 'event_completed_at'):
            ...
            if isinstance(raw_value, datetime):
                normalized_value = raw_value.replace(tzinfo=UTC) if raw_value.tzinfo is None else raw_value.astimezone(
                    UTC)
    ...
    def _mark_started(self, started_at: str | datetime | None = None) -> None:
        ...
        if isinstance(started_at, datetime):
            normalized_value = started_at.replace(tzinfo=UTC) if started_at.tzinfo is None else started_at.astimezone(
                UTC)
    ...
    async def _run_handlers(
        ...
        applicable_handlers = handlers if (handlers is not None) else eventbus._get_handlers_for_event(
            self)  # pyright: ignore[reportPrivateUsage]
        if not applicable_handlers:
            return

        pending_handler_map: dict[PythonIdStr, EventHandler | ContravariantEventHandlerCallable[Any]] = dict(
            applicable_handlers)
        ...
        if concurrency_mode == EventHandlerConcurrencyMode.PARALLEL:
            if completion_mode == EventHandlerCompletionMode.FIRST:
                ...
                for handler_id, handler_entry in applicable_handlers.items():
                    handler_tasks[asyncio.create_task(eventbus._run_handler(self, handler_entry,
                                                                            timeout=timeout))] = handler_id  # pyright: ignore[reportPrivateUsage]
            ...
            parallel_tasks = [
                asyncio.create_task(eventbus._run_handler(self, handler_entry, timeout=timeout))
    ...
    async def event_result(
        ...
        valid_results: dict[PythonIdStr, EventResult[T_EventResultType]] = {
            handler_key: event_result for handler_key, event_result in self.event_results.items() if
            include(event_result)
        }
        error_results = [
            event_result
            for event_result in self.event_results.values()
    ...
    def _mark_blocks_parent_completion_if_awaited_from_emitting_handler(self) -> None:
        ...
        if self.event_blocks_parent_completion:
            return
        from .eventbus import get_current_event, get_current_handler_id
    ...
    def event_log_tree(
        ...
        from .logging import log_event_tree
    ...
    def bus(self) -> 'EventBus':
        ...
        from .eventbus import get_current_event, get_current_eventbus, in_handler_context
...
def attr_name_allowed_on_event(key: str) -> bool:
    ...
    return key in pydantic_builtin_attrs or key in event_builtin_attrs or key.startswith(
        '_') or key in allowed_unprefixed_attrs
