...
from artanis.events.cloudevents.base import EventFactory
from artanis.events.cloudevents.event import CloudEvent
from artanis.events.cloudevents.exceptions import UnsupportedSpecVersionError
...
def get_event_factory_for_version(specversion: str) -> EventFactory:
    ...
    :param specversion: The CloudEvents specification version (e.g., "1.0")
    :return: EventFactory for the specified version (defaults to v1.0 for unknown versions)
    """
    if specversion != SPECVERSION_V1_0:
        raise UnsupportedSpecVersionError(specversion)
