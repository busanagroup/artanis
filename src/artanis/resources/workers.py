...
from artanis.exceptions import ArtanisError
...
class ArtanisWorker(SQLAlchemyWorker):
    """The worker used by Artanis Resources."""

    def __init__(self, app: "ArtanisService | None" = None):
        """Initialize the worker.

        This special worker is used to handle the repositories created by ArtanisService Resources.
    ...
    def repositories(self) -> dict[str, "SQLAlchemyTableRepository"]:
        ...
        :raises ArtanisError: If the repositories are not initialized.
        """
        if not self._resources_repositories.initialised:
            raise ArtanisError("Repositories not initialized")
