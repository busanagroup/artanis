...
from artanis.broadcast.defs import ChannelMessage, ChannelScope, ASGIReceiveCallable, ASGISendCallable, ASGIApplication
from artanis.broadcast.exceptions import StopConsumer
from artanis.broadcast.layers import BaseChannelLayer, get_channel_layer
from artanis.broadcast.utils import await_many_dispatch
...
class AsyncConsumer:
    ...
    @classmethod
    def as_asgi(cls, **init_kwargs: Any) -> Any:  # Compatible with all ASGI frameworks
        """
        Return an ASGI v3 single callable that instantiates a consumer instance
        per scope. Similar in purpose to Django's as_view().

        Args:
            **init_kwargs: Keyword arguments used to instantiate the consumer instance.
        ...
        class ASGIWrapper:
            ...
            def __init__(self, **init_kwargs: Any):
                ...
                Args:
                    **init_kwargs: Keyword arguments for consumer initialization.
                """
                self.cls = cls
