...
from artanis.broadcast.defs import ChannelMessage, ChannelCapacityDict
from artanis.broadcast.exceptions import ChannelFull
from artanis.broadcast.layers.base import BaseChannelLayer
from artanis.broadcast.layers.redis.defs import SymmetricEncryptionKeys, ChannelRawRedisHost
from artanis.broadcast.layers.redis.serializers import registry
from artanis.broadcast.layers.redis.utils import close_redis, decode_hosts, create_pool, wrap_close, consistent_hash
...
CachedRedisLayers: TypeAlias = dict[asyncio.AbstractEventLoop, RedisLoopLayer]
...
class RedisChannelLayer(BaseChannelLayer):
    ...
    def __init__(
        ...
        self.client_prefix = uuid.uuid7().hex
        ...
        self.receive_event_loop: asyncio.AbstractEventLoop | None = None
        ...
        self.receive_cleaners: list[asyncio.Future[Any]] = []
    ...
    async def receive_single(self, channel: str) -> tuple[str, ChannelMessage]:
        ...
        try:
            ...
            def _cleanup_done(_cleaner: asyncio.Task[None]) -> None:
                """Cleanup callback when a receive cleaner task completes."""
                self.receive_cleaners.remove(_cleaner)
    ...
    async def new_channel(self, prefix: str = "specific") -> str:
        ...
        """
        return f"{prefix}.{self.client_prefix}!{uuid.uuid7().hex}"
