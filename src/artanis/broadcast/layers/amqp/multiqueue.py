...
from artanis.broadcast.layers.amqp.utils import ChannelRecipient, GroupRecipient
...
class _ChannelQueue:
    ...
    def close(self) -> None:
        ...
        for waiter in self._queue._getters:
            if not waiter.done():
                waiter.set_exception(StopConsumer("Broadcaster is shutting down"))
...
class MultiQueue:
    ...
    def put_nowait(
        self,
        recipient: ChannelRecipient | GroupRecipient,
        data: Dict[str, Any],
        time: float,
        ack: Callable[[], None],
    ) -> None:
