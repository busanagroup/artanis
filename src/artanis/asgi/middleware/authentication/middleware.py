...
from artanis.asgi import exceptions
from artanis.asgi.http import APIErrorResponse
from starlette.exceptions import HTTPException
from starlette.requests import Request

from artanis.asgi.middleware.authentication.types import AccessToken
...
class AuthenticationMiddleware:
    ...
    async def _get_response(self, scope: "types.Scope", receive: "types.Receive") -> "Response | ASGIService":
        ...
        try:
            token: AccessToken = await app.injector.value(
                AccessToken, {"request": Request(scope, receive=receive)}
            )
