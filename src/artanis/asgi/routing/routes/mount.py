...
class Mount(BaseRoute):
    ...
    def _build(self, app: "BaseASGIService") -> None:
        ...
        if types.is_asgi_instance(self.app):
            self.app.parent = app
    ...
    def route_scope(self, scope: types.Scope) -> types.Scope:
        ...
        * path: The remaining path to be matched
        ...
        :return: Route scope.
        """
        is_asgi = types.is_asgi_instance(self.app)
        result = {"app": self.app if is_asgi else scope["app"]}
        ...
        if "path" in scope:
            ...
            result.update(
                {
                    "root_path": "" if is_asgi else str(url.Path(scope.get("root_path", "")) / (match.matched or "")),
