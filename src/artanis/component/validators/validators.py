...
class Check(FancyValidator):
    ...
    def validate_python(self, value, state):
        if value:
            raise Invalid(self.message('check', state), value, state)
        else:
            return value
...
class RecordIsAccessible(FancyValidator):
    messages = {
        'invalidAccess': "Insufficient right to access this record",
    }

    def validate_python(self, value, state):
        if not value:
            raise Invalid(self.message('invalidAccess', state), value, state)
        else:
            return value
