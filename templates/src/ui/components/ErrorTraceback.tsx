...
function TracebackListItem({ frame, isActive }: { frame: TFrame; isActive: boolean }) {
  ...
  return (
    <div
      className={`border-primary-300 shadow-primary-300 -mt-px border shadow-md transition duration-200 ${isActive ? 'bg-primary-200' : 'bg-primary-100 hover:bg-artanis-300 hover:shadow-artanis-300 hover:shadow-lg'}`}
    >
      <div
        className={`flex h-12 w-full cursor-pointer items-center justify-start transition duration-200 ${
          isActive
            ...
            : 'hover:border-artanis-500 border-primary-100 pl-1 hover:border-l-4 hover:pl-0'
        }`}
      >
        <div
          className={`flex h-8 w-8 items-center justify-center pl-1 ${frame.vendor ? 'text-artanis-400' : 'text-primary-500'}`}
        >
          ...
          <div>
            {filename.map((value, i) => (
              <React.Fragment key={`filename-${i}`}>
                {i > 0 && <span>/</span>}
                <span className={i === 0 && frame.vendor ? 'text-artanis-500' : ''}>{value}</span>
              </React.Fragment>
            ))}
            <span>:</span>
